...
class adminExampleBehaviors
{
	...
	{
		$favs['mailjet'] = new ArrayObject(array(
											'mailjet',
											__('Mailjet'),
											'plugin.php?p=mailjet',
											'index.php?pf=mailjet/icon.png',
											'index.php?pf=mailjet/icon-big.png',
											'usage,contentadmin',
											null,
											null));
	}
	...
	public static function adminDashboardFavsIcon($core,$name,$icon)
	{
		if ($name == 'mailjet')
		{
			...
			if ($core->blog->settings->adminExample->adminexample_active)
			{
				...
				$icon[2] = 'index.php?pf=mailjet/icon-big.png';
			}
			else
			{
				$icon[0] = __('Mailjet: inactive');
				$icon[1] = 'plugin.php?p=mailjet';
				$icon[2] = 'index.php?pf=mailjet/icon-block.png';
			}
		}
	}
}
